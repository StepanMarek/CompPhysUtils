...
def plot(datasets, axisObj, datasetLabels=False, **plotOptions):
    ...
    for dataIndex in range(len(datasets)):
        if len(datasets[dataIndex]) >= 4:
            axisObj.errorbar(datasets[dataIndex][0], datasets[dataIndex][2], xerr=datasets[dataIndex][1], yerr=datasets[dataIndex][3], capsize=4, lw=0, elinewidth=2, marker=next(plotOptions["markerstyleCycle"]), label=datasetLabels[dataIndex], color=next(plotOptions["colorCycle"]), ms=args.ms)
