...
ap.add_argument("col_triples", nargs="*", metavar="dataset index amount", default=False, help="Column coordinates and amount by which to shift the values in the column. Coordinates are given as dataset name and index, value is given as floating number. Must always come in triples. Can write m instead of -, which breaks the parser. [default : no changes to any dataset]")
        ...
        if len(args.col_triples) % 3 != 0:
            ...
            if not args.all_cols:
                if args.row_range:
                    ...
                    # By default, use all the rows present in the first translated column
                    rows = range(len(datasets[args.col_triples[0]][int(args.col_triples[1])]))
                for i in range(0,len(args.col_triples),3):
                    datasetName = args.col_triples[i]
                    columnIndex = int(args.col_triples[i+1])
                    shiftAmount = 0.0
                    if args.col_triples[i+2][0] == "m":
                        # Subsitutute a minus sign
                        shiftAmount = -float(args.col_triples[i+2][1:])
                    else:
                        shiftAmount = float(args.col_triples[i+2])
                    rows = range(len(datasets[datasetName][columnIndex]))
                    datasets[args.new_name].append([])
                    for j in rows:
                        if not args.row_range or j in args.row_range:
                            datasets[args.new_name][-1].append(datasets[datasetName][columnIndex][j] + shiftAmount)
                        else:
                            datasets[args.new_name][-1].append(datasets[datasetName][columnIndex][j])
            else:
                if args.row_range:
                    rows = args.row_range
                else:
                    # By default, use all the rows present in the first column of the source dataset
                    rows = range(len(datasets[args.col_triples[0]][0]))
                # Iterate over all columns of the source dataset
                for i in range(len(datasets[args.col_triples[0]])):
                    ...
                    for k in range(0, len(args.col_triples), 3):
                        if int(args.col_triples[k+1]) == i:
                            # We are handling a column which is being translated
                            columnTarget = True
                            shiftAmount = 0.0
                            if args.col_triples[k+2][0] == "m":
                                shiftAmount = -float(args.col_triples[k+2][1:])
                            else:
                                shiftAmount = float(args.col_triples[k+2])
                    # Handle rows of the current column
                    for j in rows:
                        if columnTarget:
